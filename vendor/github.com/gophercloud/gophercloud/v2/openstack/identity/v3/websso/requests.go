...
type AuthOptions struct {
	...
	// CallbackHost is the loopback callback host. It defaults to localhost.
	CallbackHost string

	// CallbackPort is the callback listener port. It defaults to 9990.
	CallbackPort int
...
func (opts *AuthOptions) validate() error {
	...
	if opts.CallbackPort < 0 || opts.CallbackPort > 65535 {
		return fmt.Errorf("websso: CallbackPort must be 0 or between 1 and 65535")
	}
	if opts.Timeout < 0 {
	...
	if opts.CallbackHost != "" && opts.CallbackHost != "localhost" {
		ip := net.ParseIP(opts.CallbackHost)
		if ip == nil || !ip.IsLoopback() {
			return fmt.Errorf("websso: CallbackHost must be a loopback address")
		}
	}
	return nil
}
