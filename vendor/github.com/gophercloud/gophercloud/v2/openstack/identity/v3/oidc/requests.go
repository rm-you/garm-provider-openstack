...
type AuthOptions struct {
	...
	// TokenCache enables token reuse.
...
func Create(ctx context.Context, c *gophercloud.ServiceClient, opts tokens.AuthOptionsBuilder) (r tokens.CreateResult) {
	...
	var cacheKey string
	if oidcOpts.TokenCache != nil {
		cacheKey = CacheKey(c.Endpoint, oidcOpts)
		if cached, ok := tokencache.Load(ctx, c.ProviderClient, oidcOpts.TokenCache, cacheKey, c.Endpoint); ok {
			return cached
		}
	}
	...
	if r.Err == nil && oidcOpts.TokenCache != nil {
		tokencache.Persist(oidcOpts.TokenCache, cacheKey, c.Endpoint, r)
	}
